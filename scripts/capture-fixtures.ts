...
async function fetchSample(
  ...
  const resolvedAddresses: ResolvedAddress[] =
    await resolvePublicUrlAddresses(url);
  ...
  try {
    ...
    if (
      [301, 302, 303, 307, 308].includes(response.statusCode as number) &&
      location &&
      redirectCount < 5
    ) {
    ...
    const ok =
      ...
      (response.statusCode as number) < 300;
...
const candidates = (flattenSurfaces(subnets) as Row[]).filter(
  (surface) =>
    FIXTURE_KINDS.has(surface.kind as string) &&
    surface.public_safe &&
    !surface.auth_required &&
    (surface.probe as Row | undefined)?.enabled !== false &&
    (
      ((surface.probe as Row | undefined)?.method as string) || "GET"
    ).toUpperCase() === "GET",
);
...
await mapLimit(candidates, CONCURRENCY, async (surface) => {
  const result = await fetchSample(surface.url as string);
  if (!result.ok) {
    statuses.push({
